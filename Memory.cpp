...
int Memory::GetProcessId(char* processName) {
    ...
    const size_t pSize = strlen(processName) + 1;
    wchar_t* pName = new wchar_t[pSize];
    size_t convertedChars = 0;
    mbstowcs_s(&convertedChars, pName, pSize, processName, _TRUNCATE);

    if( Process32First( hSnapshot, &pe32 ) ) {
        do {
            if(wcsicmp(pe32.szExeFile, pName ) == 0 )
                break;
    ...
    if(EnumProcessModules(processHandle, hModules, cModules/sizeof(HMODULE), &cModules)) { 
       for(size_t i = 0; i < cModules/sizeof(HMODULE); i++) { 
          if(GetModuleBaseName(processHandle, hModules[i], szBuf, sizeof(szBuf))) { 
              const size_t newSize = 50 * 2;
              char* newString = new char[newSize];
              size_t convertedChars = 0;
              wcstombs_s(&convertedChars, newString, newSize, szBuf, _TRUNCATE);
             if(sModuleName.compare(newString) == 0) { 
