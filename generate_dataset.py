...
class GameState:
    ...
    def writeSnapshot(self, snapshot, playerData, opponentData):
        with open(codes[playerData.char] + '_vs_' + codes[opponentData.char] + '.csv', 'a', newline='') as csvfile:
            writer = csv.writer(csvfile, delimiter = ',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
            writer.writerow(snapshot)

    def evaluateSnapshot(self, snapshot, playerData, opponentData, opp_classifier):
        ...
        probabilities = opp_classifier.predict_proba(snapshot[:, :-1])
        classes = opp_classifier.classes_

        option_evals = []

        for player_option in player_options:
            player_option_evaluation = None
            for opponent_option in opponent_options:
                opponent_option_weight = probabilities[classes.index(opponent_option['name'])]
                if ((opponent_option['range'] < self.dist_diff) and (player_option['range'] >= self.dist_diff)):
                    winning_option = 1
                elif ((player_option['range'] < self.dist_diff) and (opponent_option['range'] >= self.dist_diff)):
                    winning_option = -1
                elif ((player_option['range'] < self.dist_diff) and (opponent_option['range'] < self.dist_diff)):
                    winning_option = 0
                else:
                    if (player_option['startup'] > opponent_option['startup']):
                        winning_option = 1
                    elif (opponent_option['startup'] > player_option['startup']):
                        winning_option = -1
                    else:
                        winning_option = 0
                ...
                winning_option = -2 # 1 if player option wins, -1 if it loses, 0 if nothing happens
                option_eval = opponent_option_weight * winning_option
                # if (player_option_evaluation > option_eval):
                #     player_option_evaluation = option_eval
                player_option_evaluation += opponent_option_weight * winning_option # may be another way of doing this
                k = 0
        ...
        try:
            ...
            if (p2_data.actionChange == 1):
                if ((p2_action['name'] != "block") and (p2_action['name'] != "shimmy")):
                    ...
                    print(snapshot)
                    print()
                    self.writeSnapshot(snapshot, p2_data, p1_data)
                    self.p2_blocking = 0
